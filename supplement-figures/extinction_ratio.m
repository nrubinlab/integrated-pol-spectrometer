...
clear;
% --- calib for ASE data (a/d) --- %
calib_dir = 'data\20241014\calibrations\calib';
% -------------------------------- %

% --- calib for laser data --- %
%calib_dir = 'data\20241119\calib_2';
% ---------------------------- %
[~, TE_calib_file, TM_calib_file, amp_calib_file] = prompt_load_calibration(calib_dirname = calib_dir);
%% Load measurement folder
% prompt using UI

% these 6 files are what are used in the supplemental figure
% ASE
%measure_dirname = 'data\20241014\spectra_ASE\ASE_TE'; % a
%measure_dirname = 'data\20241014\spectra_ASE\ASE_TM'; % d
% 2-lasers (had to re-take this data due to lack of x-pol OSA ref)
% MAKE SURE YOU UPDATE CALIBRATION FILE ABOVE BEFORE USING THESE!
%measure_dirname = 'data\20241119\res_TE_ccw2_1'; % b
measure_dirname = 'data\20241119\res_TM_ccw2_1'; % e
%measure_dirname = 'data\20241119\res_TE_ccw2_5'; % c
%measure_dirname = 'data\20241119\res_TM_ccw2_5'; % f
%measure_dirname = uigetdir();
...
% to what polarization do we normalize the intensity?
normalize_type = "TM"; % 'TE' or 'TM'
% to roughly calibrate the difference in transmission between the two
% polarizations (as is needed for a fair PER comparison), we use this
% factor. This factor is determined by running a spectral extraction on the
% upolarized ASE source data, which should have equal TE and TM amplitudes
pol_factor = 0.36; % calibration factor from looking at unpolarized ASE

show_axis_labels = true; % we add these manually to first plot, it's easiest
lin_range = [-0.1, 1.2];
...
show_interferogram = false;
interferogram_decimate = 1;
show_osa = true;
these_x_ticks = 1480:40:1640;
%these_x_labels = ["1480", "", "1560", "", "1640"];
...
% yline produces glitchy vector output so use this workaround
dummy_x = lambda_range*1e9; dummy_y = ([1; 1]*(0:-10:-40))';
if(do_log)
    ylim([-40,0]);
    hold on;
    plot(dummy_x, dummy_y, ':', Color = '#888888');
    hold off;
