% same as generate_heater_lists but does unbalanced sweep.
% only used for one experiment in supplement
function [I_inc_out, I_dec_out, supply_interval, heater_time] = ...
    ...
    half_num = floor(acq_struct.P_num/2); % HACK ALERT
    P_diff = linspace(-acq_struct.P_range,acq_struct.P_range,half_num+1);
    P_zeros = zeros(1,half_num);
    P_inc = [P_zeros, acq_struct.P_tot/2 + P_diff/2];
    P_dec = [acq_struct.P_tot/2 - P_diff/2, P_zeros];
